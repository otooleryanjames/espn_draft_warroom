...
projections_with_source_stats <- raw_stats_all %>%
  left_join(
    projections %>% select(id, pos, points, rank, pos_rank, adp, overall_ecr),
    by = c("id", "pos")
  ) %>%
  mutate(
    player = case_when(
...
# ==========================================
# 7. INCORPORATE STRENGTH OF SCHEDULE (SoS)
# ==========================================
fp_data <- tibble(
  team = c(
  ...
  qb_sos = c(1, 5, 3, 3, 1, 3, 3, 5, 1, 3, 5, 5, 5, 5, 5, 1, 1, 1, 3, 3, 5, 1, 3, 5, 5, 5, 1, 1, 5, 1, 3, 1),
  rb_sos = c(1, 1, 3, 1, 1, 1, 1, 3, 5, 1, 5, 5, 5, 5, 5, 3, 1, 3, 5, 3, 5, 1, 3, 5, 5, 5, 3, 1, 3, 1, 3, 5),
  wr_sos = c(1, 3, 5, 5, 1, 3, 5, 3, 1, 1, 5, 5, 3, 5, 5, 5, 3, 5, 5, 5, 5, 5, 1, 3, 5, 5, 1, 3, 3, 1, 3, 1),
  te_sos = c(1, 3, 5, 3, 1, 1, 1, 3, 1, 5, 3, 1, 5, 3, 3, 5, 3, 5, 5, 5, 5, 1, 5, 1, 1, 5, 5, 5, 5, 3, 5, 5)
)
...
yahoo_data <- tibble(
  ...
  qb_rank = c(26, 12, 14, 24, 27, 31, 5,  2,  16, 21, 11, 15, 9,  8,  4,  30, 32, 23, 17, 25, 6,  19, 10, 7,  20, 1,  28, 29, 13, 22, 3,  18),
  rb_rank = c(29, 21, 12, 31, 32, 28, 27, 10, 9,  22, 1,  17, 15, 7,  14, 20, 30, 18, 4,  19, 13, 26, 3,  11, 25, 2,  23, 16, 5,  24, 8,  6),
  wr_rank = c(20, 15, 17, 14, 25, 27, 2,  3,  19, 22, 11, 12, 10, 13, 6,  29, 32, 30, 26, 28, 4,  24, 8,  7,  18, 1,  31, 23, 16, 9,  5,  21),
  te_rank = c(32, 7,  2,  22, 24, 30, 4,  3,  25, 12, 18, 29, 14, 13, 9,  5,  16, 20, 21, 26, 8,  23, 15, 10, 17, 1,  11, 27, 19, 31, 6,  28)
)

yahoo_to_5scale <- function(rank) {
  case_when(
    rank <= 6  ~ 5,
    rank <= 13 ~ 4,
    rank <= 19 ~ 3,
...
blended_sos_table <- yahoo_data %>%
  inner_join(fp_data, by = "team") %>%
  mutate(
    qb_blended = round((yahoo_to_5scale(qb_rank) + qb_sos) / 2, 1),
    rb_blended = round((yahoo_to_5scale(rb_rank) + rb_sos) / 2, 1),
    wr_blended = round((yahoo_to_5scale(wr_rank) + wr_sos) / 2, 1),
    te_blended = round((yahoo_to_5scale(te_rank) + te_sos) / 2, 1)
  ) %>%
...
weighted_projections <- weighted_projections %>%
  ...
  left_join(sos_long, by = c("team", "pos")) %>%
  mutate(
